...
def a_bar(conn, security_id, day, an_observation, close="100"):
    conn.execute(
...
def test_a_security_held_to_yesterday_fetches_only_the_settling_window(
    fresh_db, an_observation
):
    sid = a_security(fresh_db)
    a_bar(fresh_db, sid, date(2026, 9, 4), an_observation)
...
def test_a_security_three_days_stale_fetches_the_gap_plus_the_window(
    fresh_db, an_observation
):
    sid = a_security(fresh_db)
    a_bar(fresh_db, sid, date(2026, 9, 1), an_observation)
    assert windows(fresh_db, [sid], today=TODAY) == {sid: settling_cutoff(TODAY)}


def test_a_security_stale_by_a_month_fetches_back_to_where_it_stopped(
    fresh_db, an_observation
):
    sid = a_security(fresh_db)
    a_bar(fresh_db, sid, date(2026, 8, 1), an_observation)
...
def test_every_requested_security_gets_an_answer(fresh_db, an_observation):
    held = a_security(fresh_db, "AAPL")
    fresh = a_security(fresh_db, "MSFT")
    a_bar(fresh_db, held, date(2026, 8, 1), an_observation)
